    ...
    run: async (client, message, args) => {
        ...
        if (!message.member.permissions.has("BAN_MEMBERS")) return message.channel.send(
            new MessageEmbed()
                ...
                .setDescription("**You Dont Have The Permissions To MassBan Users! - [BAN_MEMBERS]**")
        ...
        message.channel.send(` **Are you sure you want to ban ${bannedCollection.map(mem => `${mem.toString()}`).join(", ")}?** \nPlease reply with \`y\`/\`yes\` or \`cancel\``)
        collector.on("collect", async msg => {
            if (['yes', 'y'].includes(msg.content.toLowerCase())) {
                ...
                collector.stop()
                return message.channel.send("Massban", "I have cancelled the massban on " + bannedCollection.size + " members")
            } else {
                return message.channel.send(`**PROMPT FAILED**: \n Are you sure you want to ban ${bannedCollection.map(mem => `**${mem.user.username}**`).join(", ")}? \nPlease reply with \`y\`/\`yes\` or \`cancel\``)
