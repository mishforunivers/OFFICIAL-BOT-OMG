...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if (!message.member.hasPermission("ADMINISTRATOR") && !ownerID.includes(message.author.id)) return message.channel.send("You Don't Have Sufficient Permissions!- [MANAGE_GUILD]");

    let serverRegion = args.slice(0).join(' ')

    if(!serverRegion) return message.channel.send("Please provide a region! OR use ??help svr to know region ID");
    ...
    if(availableRegions.includes(serverRegion)) {
      ...
      return message.channel.send(`Checking for error.....`).then((msg) => {
          setTimeout(() => {
