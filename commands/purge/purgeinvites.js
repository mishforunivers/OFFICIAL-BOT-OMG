const {
  ...
  module.exports = {
    name: 'purge-invites',
    description: 'Deletes an amount of messages containing invite links in the current channel!',
    aliases: ["purgeinvites", "clear-invites"],
    ...
    run: async (client, message, args) => {
      ...
      let messages = (await channel.messages.fetch({
        limit: amount
      })).filter(m => m.content.startsWith('https://discord.gg') || m.content.includes('https://discord.gg'));
      ...
      if (messages.size === 0) {
        ...
        message.channel.send(
          ...
          .setTitle('Purge')
          .setDescription(`
                  Unable to find any messages containing invite links. 
        ...
        channel.bulkDelete(messages, true).then(messages => {
          const embed = new MessageEmbed()
            .setTitle('Purge')
            .setDescription(`
                  Successfully deleted **${messages.size}** message(s) containing invite links. 
                  This message will be deleted after \`10 seconds\`.
                `)
            .addField('Channel', channel, true)
            .addField('Message Count', `\`${messages.size}\``, true)
            .addField("Requirement: ", `Must have an invite link`, true)
            .setFooter(message.member.displayName, message.author.displayAvatarURL({
              dynamic: true
            }))
            .setTimestamp()
